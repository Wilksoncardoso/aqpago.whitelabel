<template lang="html">
  <div>
    <div class="body_recorrente white">
        ...
        <div class="d-flex-inline text-center" style="margin-top: 55px">
          ...
          <div class="card_cobranca_gerada mt-8">
            ...
            <div class="value_gerada primary--text mt-2">
              R${{ $formatMoney(0 || result_preview?.data?.valor_original) }}
            </div>
          </div>
          <!-- <div class="card_cobranca_gerada mt-8">
          ...
          <div class="card_share_cobranca my-12">
            ...
            <div class="d-flex justify-space-around">
              ...
              <v-btn
                ...
                :loading="loading_set[result_preview?.data?.id]"
                v-if="parseFloat(result_preview?.data?.valor_original) >= 10 && result_preview?.data?.pagamento_boleto"
                @click="
                  geracao_boleto_pai(
                    result_preview?.data?.id,
                    result_preview?.data
                  )
                "
...
export default {
  ...
  computed: {
    ...
    notificacao() {
      const social = {
        whatsapp: this.result_preview?.data?.notificar_whatsapp,
        sms: this.result_preview?.data?.notificar_sms,
        email: this.result_preview?.data?.notificar_email,
      };
      return social;
    },
  ...
  methods: {
    ...
    copyToClipboard() {
      ...
      textarea.value = this.url + this.result_preview?.data?.url;
    ...
    async geracao_boleto_filha(id, id_pai, meta) {
      const data = meta;
      const compradores = this.compradores[0];
      let name =
