  ...
  <div>
    <v-dialog class="modal_z_index_all" v-model="dialog" width="464">
      <v-card class="token__transacao">
        ...
        <p class="text__token">
          Digite o código que foi enviado para seu novo celular.
        </p>
        <v-otp-input
          class="input__token"
          v-model="form.token"
...
export default {
  ...
  created() {
    this.$nuxt.$on("edit__tel", ($event) => this.edit__tel($event));
  },
  methods: {
    onFinish() {
    ...
    concluir__transacao() {
      ...
      this.$axios
        ...
        .then((response) => {
          this.$toast.success("celular alterado!");
          var next__page = 0;
          this.form.token = "";
