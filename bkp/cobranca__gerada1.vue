  ...
  <div class="card__extrato__detalhado__">
    ...
    <v-row id="img__comprovante">
      <v-col cols="12" lg="3" xl="3">
        ...
        <p class="titulo_extrato">{{ body_cobranca.payer }}</p>
        <p class="_extrato">
      ...
      <v-col cols="12" lg="4" xl="4">
        ...
        <p class="titulo_extrato" :class="body_cobranca.status">
          {{ status__pix }}
        </p>
      </v-col>
      ...
      <v-col cols="12" lg="9" xl="9" v-if="body_cobranca.descripition != null">
    ...
    <v-row>
      <v-col cols="12" lg="3" xl="3">
        <qr-code class="qr__code" :value="body_cobranca.brcode" :options="{ width: '100%' }"></qr-code>
      </v-col>
      <v-col cols="12" lg="9" xl="9">
        <p class="label_extrato">Código:</p>
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <div class="cod__cobranca" v-bind="attrs" v-on="on" @click="copyToClipboard()">
              {{ body_cobranca.brcode }}
            </div>
          </template>
          <span>{{ label }}</span>
        </v-tooltip>
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn v-bind="attrs" v-on="on" class="pa-0 button__copy" color="primary" text @click="copyToClipboard()">
              <div class="d-flex">
                <div class="icon">
                  <icon__copy />
                </div>
                <div class="copiar_cod">Copiar código</div>
              </div>
            </v-btn>
          </template>
          <span>{{ label }}</span>
        ...
        <div class="d-flex align-center">
          ...
          <div>
            <h4 class="text-left" style="margin-right: 10px">Compartilhar</h4>
          </div>

          <div class="card__compartilhar1">
            <div class="d-flex justify-space-between">
              <div class="d-flex">
                <div>
                  <v-btn class="button__compartilhar telegram" target="_blank" :href="'https://t.me/share/url?url=' +
                    ...
                    body_cobranca.payer +
                    ' - ' +
                    body_cobranca.invoice_name +
                    ', ' +
                    body_cobranca?.descripition +
                    ' '
                    ">
                    <icon__telegram />
                  </v-btn>
                ...
                <div>
                  <v-btn class="button__compartilhar whatsapp" target="_blank" :href="'https://api.whatsapp.com/send?text=' +
                    body_cobranca.payer +
                    ' - ' +
                    body_cobranca.invoice_name +
                    ', ' +
                ...
                <div>
                  <v-btn class="button__compartilhar email" @click="dialog = true">
                    <icon__email />
                  </v-btn>
                </div>
              </div>
              <!-- <div class="d-flex">
                <div>
                  <v-btn outlined class="button__right__compartilhar down normal" color="primary" style="height: 44px"
                    @click="comprovante__download()">
                    <icon__download class="mr-1" /> Baixar
                  </v-btn>
                </div>
                <a :href="imageUrl" download ref="downloadLink" style="display: none">Baixar imagem</a>
    ...
    <div class="d-flex justify-center">
      <div class="d-flex-inline">
        ...
        <div>
          <p class="label_extrato text font-weight-bold">{{ body_cobranca.id }}</p>
        </div>
      </div>
    </div>
    ...
    <v-dialog class="modal_z_index_all" v-model="dialog" width="500">
      <v-card class="px-10 py-6">
        ...
        <v-form ref="form" v-model="valid" lazy-validation>
          ...
          <v-text-field v-model="form.destinatario" :counter="20" :rules="rules.nameRules" class="input__padrao"
            label="Nome de destinatário" required solo></v-text-field>
          <p class="label__padrao">E-mail:</p>

          <v-text-field solo label="Digite o e-mail do operador" class="input__padrao" v-model="form.email"
            :rules="rules.emailRules">
          </v-text-field>
          <div class="d-flex justify-end">
            <v-btn :loading="loading1" :disabled="!valid" class="button__default" color="primary" @click="validate()">
              Enviar
            </v-btn>
          </div>
        </v-form>
