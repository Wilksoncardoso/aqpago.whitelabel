export default ({ app }, inject) => {
  ...
  const maskPix = (type, value) => {
    switch (type) {
      case "cpf":
        return maskDoc(value);
  ...
  const ListTreeText = (text) => {
   
    if (!text) return "";
    let nomes = text.trim().split(" ").filter(nome => nome.length > 0);
    if (nomes.length === 0) return "";
    if (nomes.length === 1) return nomes[0];
    if (nomes.length <= 3) {
      return nomes.join(" ");
    }
    // Para mais de 3 palavras
    const primeiro = nomes[0];
    const meio = nomes[Math.floor(nomes.length / 2)];
    const ultimo = nomes[nomes.length - 1];

    return `${primeiro} ${meio} ${ultimo}`;
  }

  const MascDocDefault = (value) => {
  ...
  inject('ListTreeText', ListTreeText)
  inject('MascDocDefault', MascDocDefault)
}
