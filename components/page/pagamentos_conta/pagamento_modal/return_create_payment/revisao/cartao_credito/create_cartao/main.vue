<template lang="html">
  <div>
    <div class="label_card mb-4">Adicionar cartão</div>
    <div class="card_body">
...
export default {
  components: {
    mastercard: () =>
      import("~/components/page/recarga/body/icon/bandeira/mastercard.vue"),
    elo: () => import("~/components/page/recarga/body/icon/bandeira/elo.vue"),
    visa: () => import("~/components/page/recarga/body/icon/bandeira/visa.vue"),
    amex: () =>
      import(
        "~/components/page/recarga/body/icon/bandeira/american-express.vue"
      ),
    hipercard: () =>
      import("~/components/page/recarga/body/icon/bandeira/hipercard.vue"),
    hiper: () =>
      import("~/components/page/recarga/body/icon/bandeira/hiper.vue"),
    default: () =>
      import("~/components/page/recarga/body/icon/bandeira/default.vue"),
    jcb: () => import("~/components/page/recarga/body/icon/bandeira/jcb.vue"),
    Diners: () =>
      import("~/components/page/recarga/body/icon/bandeira/dinersclub.vue"),
    indicador__select: () =>
      import("~/components/page/recarga/body/icon/dwon_select.vue"),
  },
  directives: { mask },
  props: {
    credits: {
  ...
  methods: {
    ...
    return_list_principal() {
      this.$nuxt.$emit("variable_data_comp", { type: "retun_listing" });
    },
    ...
    async next_page() {
      ...
      const response = await this.$axios
        ...
        .then((res) => {
          this.result_cartao = res;

          setTimeout(() => {
            this.loading = false;
            const data = {
              type: "save_dados_credit",
              data_credit: this.result_cartao,
              cvv: this.form_out.cvc,
            };
            this.$nuxt.$emit("variable_data_comp", data);
          }, 3000);
        })
        .catch((error) => {
          this.error =
    ...
    init__variable() {
      ...
      this.form.prop_cpf = this.data_user?.cpf;
      this.mask.document = this.$Formdoc(this.data_user?.cpf);
    },
  },
  data() {
    return {
      ...
      result_cartao: {
        success: true,
        msg: "cartao gerado com sucesso",
        body: {
          id: 85858,
          tipo_cartao: "credito",
          cpf_proprietario: "02130412270",
          nome_proprietario: "procode ltda",
          numero: "549738xxxxxx5716",
          bandeira: "MasterCard",
          pagamento_validate_cartao: null,
          pagamento_validacao_check: 0,
          validade_mes: "01",
          verificacao_cartao: null,
          validate_tentativas: null,
          validade_ano: "2031",
          apelido_cartao: null,
          endereco: null,
          created_at: "2025-01-09 15:17:26",
          updated_at: "2025-01-09 15:17:26",
        },
      },
      form: {
        prop_cpf: "",
