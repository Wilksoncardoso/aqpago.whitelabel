    ...
    >
      <v-card class="extrato__modal__principal">
        <div class="card__filter">
          ...
          <h2 class="titulo__">Filtro vendas</h2>
...
export default {
  ...
  created() {
    this.$nuxt.$on("modal__filtro__vendas", ($event) =>
      this.modal__filtro__vendas($event)
    );
  },
  methods: {
    clean() {
    ...
    modal__filtro__vendas($event) {
      this.dialog = $event.modal__filtro;
    },
    Filter__api__result__venda() {
      // busca por filtro
      this.$nuxt.$emit("variavel__filter__vendas", { form: this.form });
    },
