    ...
    <v-dialog v-model="dialog" max-width="760">
      <v-card class="pa-6 modal-creted-seller" style="border-radius: 20px">
        <div class="d-flex align-center justify-space-between mb-4">
          <h2 class="mb-0">Updated de cliente</h2>
              ...
              <v-row class="py-2">
                <v-col cols="12" md="6" class="py-0">
                  ...
                  <v-text-field
                    ...
                    label="CNPJ*"
                    v-model="einMasked"
                    :rules="[rules.required, rules.cnpjDigits]"
                    v-mask="['##.###.###/####-##']"
                    maxlength="18"
                    ref="r_ein"
                  />
...
export default {
  ...
  methods: {
    ...
    SetFormMask(data) {
      this.page = 1;
      this.einMasked = String(data?.ein || "")
        .replace(/\D/g, "")
        .slice(0, 14);
      this.businessPhoneMasked = String(data?.business_phone || "")
        .replace(/\D/g, "")
        .slice(0, 11);
      this.businessCepMasked = String(data?.business_address.zip_code || "")
        .replace(/\D/g, "")
        .slice(0, 8);
      this.ownerCpfMasked = String(data?.owner.taxpayer_id || "")
        .replace(/\D/g, "")
        .slice(0, 11);
      this.ownerPhoneMasked = String(data?.owner.phone || "")
        .replace(/\D/g, "")
        .slice(0, 11);
      this.ownerCepMasked = String(data?.owner.address.zip_code || "")
        .replace(/\D/g, "")
        .slice(0, 8);
      this.name_full = String(
    ...
    async submit() {
      ...
      // postaria this.form aqui
      // await this.$axios.$post('/api/clientes', this.form)
